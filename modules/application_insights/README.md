...
| daily_data_cap_in_gb | number | no | The maximum amount of data (in gigabytes) that can be ingested per day before ingestion is throttled. Helps control costs by limiting daily data volume. |
...
| disable_ip_masking | bool | no | Whether to disable automatic masking of IP addresses in telemetry data. Set to true to store full IP addresses instead of masked versions for privacy. |
| force_customer_storage_for_profiler | bool | no | Whether to require customer-provided storage accounts for Application Insights Profiler data instead of using Microsoft-managed storage. |
...
| local_authentication_disabled | bool | no | Whether to disable authentication using instrumentation keys or connection strings. When true, only Azure Active Directory authentication is allowed. |
| retention_in_days | number | no | The number of days that telemetry data will be retained in Application Insights before being automatically deleted. Common values are 30, 60, 90, 120, 180, 270, 365, 550, or 730 days. |
| sampling_percentage | number | no | The percentage of telemetry data to collect and store, from 0 to 100. Lower values reduce data volume and costs but may miss some telemetry events. |
| workspace_id | string | no | The ID of the Log Analytics workspace to use as the backing store for this Application Insights instance. If not specified, a classic Application Insights resource is created instead. |
